...
class AndroidTheme(private val strategy: AndroidStrategy) : Theme<AndroidStrategy, AndroidPen, AndroidPath> {
    private val pens: SparseArray<SparseArray<AndroidPen>>

    init {
        pens = SparseArray()
    }

    override fun getPen(item: ThemeItem, state: Int): AndroidPen {
        val itemState = item.getEffectiveState(state)
        val themeState = overrideState(item, state, itemState)
        var statePens: SparseArray<AndroidPen>? = pens.get(item.itemNo)
        if (statePens == null) {
            statePens = SparseArray()
            pens.append(item.itemNo, statePens)
        }

        var result: AndroidPen? = statePens.get(themeState)
        if (result == null) {
            result = item.createPen(strategy, itemState)
    ...
    companion object {
        ...
        @JvmField
        val TOUCHED_SHADE_COLOR = Color.argb(0xb0, 0xff, 0xec, 0x1a)
        @JvmField
        val SELECTED_SHADE_COLOR = Color.argb(0xb0, 23, 166, 255)
         ...
         */
        private fun overrideTheme(pen: AndroidPen, item: ThemeItem, state: Int): AndroidPen {
            if (item is ProcessThemeItems) {
                when (item) {
