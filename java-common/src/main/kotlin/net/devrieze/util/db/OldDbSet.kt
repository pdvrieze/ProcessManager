...
open class OldDbSet<T:Any>(pTransactionFactory: TransactionFactory<OldDBTransaction>, protected open val elementFactory: OldElementFactory<T, OldDBTransaction>) : AutoCloseable, Closeable {
  ...
  fun <T:Any> toArray(pA: Array<T>): Array<T> {
    try {
      (unsafeIterator(true) as ResultSetIterator).use { it ->
        val size = it.size()
