...
object Connectors {
  ...
  private const val HEADANGLE = 35 * Math.PI / 180
  private val HEADDX = Math.cos(HEADANGLE) * HEADLEN
  private val HEADDY = Math.sin(HEADANGLE) * HEADLEN
  private const val MINANGLE = 1 * Math.PI / 180
  ...
  fun <PEN_T : Pen<PEN_T>,
    ...
    val miterExtend = 0.5 * pen.strokeWidth / Math.sin(HEADANGLE)
    ...
    val headStartX = (headControlX + Math.cos(lineAngle) * HEAD_CONTROL_DIST).toFloat()
    val headStartY = (headOffsetY + Math.sin(lineAngle) * HEAD_CONTROL_DIST).toFloat()
    val tailEndX = (tailJoinX - Math.cos(lineAngle) * TAIL_JOIN_DIST).toFloat()
    val tailEndY = (tailJoinY - Math.sin(lineAngle) * TAIL_JOIN_DIST).toFloat()
    val tailControlX2 = (tailEndX + Math.cos(lineAngle) * TAIL_CONTROL_DIST).toFloat()
    val tailControlY2 = (tailEndY + Math.sin(lineAngle) * TAIL_CONTROL_DIST).toFloat()
  ...
  fun <PATH_T : DiagramPath<PATH_T>,
    ...
    val miterExtend = 0.5 * pen.strokeWidth / Math.sin(HEADANGLE)
    val miterExtendX = (Math.cos(angle) * miterExtend).toFloat()
    val miterExtendY = (Math.sin(angle) * miterExtend).toFloat()

    val arrowPath = strategy.newPath()

    val headLen = Math.sqrt(HEADDX * HEADDX + HEADDY * HEADDY)
    ...
    var headAngle = angle + Math.PI - HEADANGLE
    val headDX1 = (Math.cos(headAngle) * headLen).toFloat()
    val headDY1 = (Math.sin(headAngle) * headLen).toFloat()
    headAngle = angle + Math.PI + HEADANGLE
    val headDX2 = (Math.cos(headAngle) * headLen).toFloat()
    val headDY2 = (Math.sin(headAngle) * headLen).toFloat()
