...
import net.devrieze.util.CollectionUtil.combine
...
open class LayoutAlgorithm {
  ...
  open fun <T: Positioned> layout(nodes: List<DiagramNode<T>>, layoutStepper: LayoutStepper<T> = AbstractLayoutStepper()): Boolean {
    ...
    for (partition in partition(nodes)) {
      changed = layoutPartition(partition, bottomY, layoutStepper) || changed
  ...
  private inner class Instance<T:Positioned>(val layoutStepper: LayoutStepper<T>) {
    ...
    fun layoutPartition(nodes: List<DiagramNode<T>>, topY: Double): Boolean {
      ...
      for (node in nodes) {
        if (! node.hasValidPosition) {
          changed = layoutNodeInitial(nodes, node, topY) or changed // always force as that should be slightly more efficient
        }
    ...
    private fun layoutNodeInitial(nodes: List<DiagramNode<T>>,
      ...
      for (node in combine(leftNodes, aboveNodes)) {
        if (! node.hasValidPosition) {
    ...
    private fun List<DiagramNode<T>>.verifyPositions(): Boolean {
      ...
      for (node in this) {
        ...
        val minY = bottom(nodesAbovePos(this, node).lowest(), Double.NEGATIVE_INFINITY) + horizSeparation + node.topExtent
    ...
    private fun Iterable<DiagramNode<T>>.lowest(report: Boolean = true)
      = maxBy { it.bottom.onNaN(Double.NEGATIVE_INFINITY) }?.also { if (report) layoutStepper.reportLowest(this.toList(), it) }


    private fun Iterable<DiagramNode<T>>.highest(report: Boolean = true)
      = minBy { it.top.onNaN(Double.POSITIVE_INFINITY) }?.also { if (report) layoutStepper.reportHighest(this.toList(), it) }

    private fun Iterable<DiagramNode<T>>.leftMost(report: Boolean = true)
      = minBy { it.left.onNaN(Double.NEGATIVE_INFINITY) }?.also { if (report) layoutStepper.reportLeftmost(this.toList(), it) }


    private fun Iterable<DiagramNode<T>>.rightMost(report: Boolean = true)
      = maxBy { it.right.onNaN(Double.POSITIVE_INFINITY) }?.also { if (report) layoutStepper.reportRightmost(this.toList(), it) }
    ...
    private fun nodesAbovePos(nodes: List<DiagramNode<T>>, node: DiagramNode<T>)
      = nodes.filter { n -> n upOverlaps node }

    private fun nodesBelowPos(nodes: List<DiagramNode<T>>, node: DiagramNode<T>)
      = nodes.filter { it downOverlaps node }

    private fun nodesLeftPos(nodes: List<DiagramNode<T>>, node: DiagramNode<T>)
      = nodes.filter { it leftOverlaps node }

    private fun nodesRightPos(nodes: List<DiagramNode<T>>, node: DiagramNode<T>)
      = nodes.filter { it rightOverlaps node }

    private inline fun moveCommon(nodes: List<DiagramNode<T>>, filter:(DiagramNode<T>)->Boolean, update: (DiagramNode<T>)->Unit): Boolean {
    ...
    private fun moveToRight(nodes: List<DiagramNode<T>>, freeRegion: DiagramNode<T>): Boolean {
      return moveCommon(nodes, { it rightOverlaps freeRegion}, { it.x = freeRegion.right+horizSeparation+it.leftExtent })
    }

    private fun moveDown(nodes: List<DiagramNode<T>>, freeRegion: DiagramNode<T>): Boolean {
      return moveCommon(nodes, { it downOverlaps  freeRegion}, { it.y = freeRegion.bottom+vertSeparation+it.topExtent })
    }
