...
class ComposeTheme(private val strategy: ComposeStrategy) : Theme<ComposeStrategy, ComposePen, ComposePath> {
    private val pens: MutableList<MutableList<ComposePen?>?>

    init {
        pens = mutableListOf()
    }

    override fun getPen(item: ThemeItem, state: Int): ComposePen {
    ...
    companion object {
         ...
         */
        private fun overrideTheme(pen: ComposePen, item: ThemeItem, state: Int): ComposePen {
            if (item is ProcessThemeItems) {
                when (item) {
