...
interface Canvas<S : DrawingStrategy<S, PEN_T, PATH_T>, PEN_T : Pen<PEN_T>, PATH_T : DiagramPath<PATH_T>> {
  ...
  enum class TextPos {
    ...
    fun offset(rect: Rectangle, pen: Pen<*>) {
      ...
      when (this) {
        ...
        ASCENTLEFT, ASCENT, ASCENTRIGHT             -> rect.top += pen.getTextAscent() - pen.getTextMaxDescent()
        LEFT, MIDDLE, RIGHT                         -> rect.top += (pen.getTextMaxAscent() + pen.getTextMaxDescent()) / 2 - pen.getTextMaxAscent()
        BASELINELEFT, BASELINEMIDDLE, BASELINERIGHT -> rect.top -= pen.getTextMaxAscent()
        DESCENTLEFT, DESCENT, DESCENTRIGHT          -> rect.top += pen.getTextDescent() - pen.getTextMaxAscent()
        BOTTOMLEFT, BOTTOM, BOTTOMRIGHT             -> rect.top += pen.getTextMaxDescent() - pen.getTextMaxAscent()
      }//
   ...
   */
  @Deprecated("")
  fun drawPoly(points: DoubleArray, color: PEN_T)
