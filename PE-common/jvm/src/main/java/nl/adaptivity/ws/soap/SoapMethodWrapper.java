...
public class SoapMethodWrapper extends WsMethodWrapper {
  ...
  private void processSoapBody(@NotNull final org.w3.soapEnvelope.Envelope<ICompactFragment> envelope, @SuppressWarnings("unused") final Map<String, DataSource> attachments) throws XmlException {
    ...
    this.setParams(new Object[parameterTypes.length]);
    ...
    for (int i = 0; i < parameterTypes.length; ++i) {
      ...
      if ((annotation != null) && annotation.header()) {
        if (parameterTypes[i].isAssignableFrom(Principal.class) && envelope.getHeader().getPrincipal()!=null) {
            this.getParams()[i] = envelope.getHeader().getPrincipal();
          continue; //Finish the parameter, we don't need to unmarshal
            ...
            this.getParams()[i] = envelope.getHeader().getPrincipal().getName();
          continue;
        } else {
      ...
      final SoapSeeAlso seeAlso = Annotations.getAnnotation(parameterAnnotations[i], SoapSeeAlso.class);
        this.getParams()[i] = SoapHelper.unMarshalNode(getMethod(), parameterTypes[i], seeAlso == null ? new Class<?>[0] : seeAlso.value(), parameterAnnotations[i], value);
  ...
  private void assertRootNode(@NotNull final XmlReader reader) throws XmlException {
    final WebMethod wm = getMethod().getAnnotation(WebMethod.class);
    if ((wm == null) || wm.operationName().equals("")) {
      if (!StringUtil.isEqual(reader.getLocalName(), getMethod().getName())) {
  ...
  public Source getResultSource() {
    ...
    try {
      return SoapHelper.createMessage(new QName(getMethod().getName() + "Response"), headers, params);
