...
package uk.ac.bournemouth.util.kotlin.sql.old
...
inline fun <R> connection(conn:Connection, block: (ConnectionHelper) -> R):R {
  conn.autoCommit=false
  try {
    return block(ConnectionHelper(conn)).apply { System.err.println("commit"); conn.commit() }
  } catch(e:Exception) {
...
fun <R> DataSource.connection(username: String, password: String, block: (ConnectionHelper) -> R) = getConnection(username, password).use { connection(it, block) }
...
class ConnectionHelper constructor(val connection: Connection) {
  ...
  fun <R> use(block: (ConnectionHelper) -> R): R = useHelper({ it.connection.close() }) {
   ...
   */
  fun <R> prepareStatement(@Language("SQL") sql: String, block: StatementHelper.() -> R): R = connection.prepareStatement(sql).use { block(StatementHelper(it)) }
   ...
   * * or this method is called on a closed connection
   */
  private inline fun prepareCall(@Language("SQL") sql: String) = connection.prepareCall(sql)
  ...
  inline fun nativeSQL(@Language("SQL") sql: String): String = connection.nativeSQL(sql)
   ...
   */
  @Throws(SQLException::class)
  fun <R> prepareStatement(@Language("SQL") sql: String, resultSetType: Int,
   ...
   */
  @Throws(SQLException::class)
  fun <R> prepareStatement(@Language("SQL") sql: String, autoGeneratedKeys: Int, block: (StatementHelper) -> R): R {
   ...
   */
  @Throws(SQLException::class)
  fun <R> prepareStatement(@Language("SQL") sql: String, columnIndexes: IntArray, block: (StatementHelper) -> R): R {
   ...
   */
  @Throws(SQLException::class)
  fun <R> prepareStatement(@Language("SQL") sql: String, columnNames: Array<out String>, block: (StatementHelper) -> R): R {
...
//  inline fun <R> params(block: ParamHelper_.() -> R) = ParamHelper_(this).block()
  ...
  fun <R> withResultSet(block: (ResultSet) -> R) = statement.getResultSet().use(block)

  fun <R> withGeneratedKeys(block: (ResultSet) -> R) = statement.generatedKeys.use(block)

  fun <R> execute(block: (ResultSet) -> R) = statement.executeQuery().use(block)

  inline fun <R> executeEach(crossinline block: (ResultSet) -> R): List<R> = execute { resultSet ->
    ArrayList<R>().apply {
      add(block(resultSet))
    }
  }
 ...
 */
//private inline fun <T : Connection, R> T.use(block: (T) -> R) = useHelper({ it.close() }, block)

//private inline fun <T : Statement, R> T.use(block: (T) -> R) = useHelper({ it.close() }, block)
//private inline fun <T : ResultSet, R> T.use(block: (T) -> R) = useHelper({ it.close() }, block)

private inline fun <T : Connection, R> T.useTransacted(block: (T) -> R): R = useHelper({ it.close() }) {
  it.autoCommit = false
  try {
    val result = block(it)
    it.commit()
...
//
//private inline fun  <T, R> T.useHelper(close: (T) -> Unit, block: (T) -> R): R {
//  var closed = false
//  try {
//    return block(this)
//  } catch (e: Exception) {
