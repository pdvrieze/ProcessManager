...
actual enum class EventType {
    START_DOCUMENT(isIgnorable = true) {
        ...
        actual override fun writeEvent(writer: XmlWriter, reader: XmlReader) =
    ...
    ENTITY_REF {
        ...
        @Throws(XmlException::class)
        actual override fun writeEvent(writer: XmlWriter, textEvent: XmlEvent.TextEvent) = writer.entityRef(textEvent.text)
    ...
    IGNORABLE_WHITESPACE(isIgnorable = true) {
        ...
        @Throws(XmlException::class)
        actual override fun writeEvent(writer: XmlWriter, textEvent: XmlEvent.TextEvent) = writer.ignorableWhitespace(
            textEvent.text)
    ...
    PROCESSING_INSTRUCTION(isIgnorable = true) {
        ...
        actual override fun writeEvent(writer: XmlWriter, textEvent: XmlEvent.TextEvent) = writer.processingInstruction(
            textEvent.text)
    ...
    actual val isIgnorable: Boolean
    ...
    @Throws(XmlException::class)
    actual open fun writeEvent(writer: XmlWriter, textEvent: XmlEvent.TextEvent): Unit = throw UnsupportedOperationException(
        "This is not generally supported, only by text types")

    @Throws(XmlException::class)
    actual abstract fun writeEvent(writer: XmlWriter, reader: XmlReader)

    @Throws(XmlException::class)
    actual abstract fun createEvent(reader: XmlReader): XmlEvent
