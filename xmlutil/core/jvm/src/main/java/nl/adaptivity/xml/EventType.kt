...
actual enum class EventType
{
  ...
  IGNORABLE_WHITESPACE(isIgnorable = true) {
    ...
    @Throws(XmlException::class)
    actual
    override fun writeEvent(writer: XmlWriter, textEvent: XmlEvent.TextEvent) = writer.ignorableWhitespace(textEvent.text)
  ...
  PROCESSING_INSTRUCTION(isIgnorable = true) {
    ...
    @Throws(XmlException::class)
    actual
    override fun writeEvent(writer: XmlWriter, textEvent: XmlEvent.TextEvent) = writer.processingInstruction(textEvent.text)
