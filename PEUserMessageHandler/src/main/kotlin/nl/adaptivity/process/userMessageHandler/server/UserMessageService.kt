...
class UserMessageService<T : Transaction> private constructor(
    ...
    taskMap: IMutableUserTaskMap<T>
) : CompletionListener<Boolean>/*<TODO Placeholder type*/ {
    ...
    private class MyDBTransactionFactory internal constructor(private val context: Context) :
        ...
        override fun asTransaction(dbReceiver: DBReceiver<UserTaskDB>): MonadicDBTransaction<UserTaskDB> = when (dbReceiver){
            is MonadicDBTransaction -> dbReceiver
            else -> MonadicDBTransaction(
                MonadicDBConnection((dbReceiver as ConnectionSource).getRawConnection(), UserTaskDB)
            )
        }
        ...
        override fun <R> inTransaction(action: suspend MonadicDBTransaction<UserTaskDB>.() -> R): R {
    ...
    private object InstantiationHelper {
        ...
        val INSTANCE: UserMessageService<MonadicDBTransaction<UserTaskDB>> = UserMessageService.newDBInstance()
    ...
    fun <R> inTransaction(@BuilderInference block:  UserMessageServiceTransaction.() -> R): R {
        return transactionFactory.inTransaction {
            UserMessageServiceTransaction(this).block()
        }
    }
    ...
    companion object {
        ...
        @JvmStatic
        fun newDBInstance(): UserMessageService<MonadicDBTransaction<UserTaskDB>> {
        ...
        @JvmStatic
        fun <T : Transaction> newTestInstance(
            transactionFactory: TransactionFactory<T>,
            taskMap: IMutableUserTaskMap<T>
        ): UserMessageService<T> {
            return UserMessageService(transactionFactory, taskMap)
        }

        @JvmStatic
        val instance: UserMessageService<MonadicDBTransaction<UserTaskDB>>
