...
class SimpleNamingContext : Context {
    ...
    private fun getParent(name: Name): Context {
        return lookup(name.getPrefix(name.size() - 1)) as? Context
            ?: throw NotContextException("The name $name does not resolve to a context")
    }
    ...
    override fun lookup(name: Name): Any? {
        return name.asSequence().fold<String, Any?>(this) { obj: Any?, component ->
            ...
            if (obj is SimpleNamingContext) {
                ...
                resolvedObject
                    ?: throw NameNotFoundException("No element with name $component (from $name) could be found in the context")
            } else {
